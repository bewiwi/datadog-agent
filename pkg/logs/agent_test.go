...
func createAgent(serverConfig *config.ServerConfig) (*Agent, *config.LogSources, *service.Services) {
...
func (suite *AgentTestSuite) TestAgent() {
	...
	host, port := mock.AddrToHostPort(l.Addr())
	serverConfig := config.NewServerConfig(host, port, false)

	agent, sources, _ := createAgent(serverConfig)
...
func (suite *AgentTestSuite) TestAgentStopsWithWrongBackend() {
	serverConfig := config.NewServerConfig("fake:", 0, false)
